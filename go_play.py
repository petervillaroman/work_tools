...
def get_confirmation(from_mode, to_mode):
    ...
    print("3. Restore your previous personal session")
...
def load_personal_state():
    """Load and restore personal state"""
    try:
        if not get_confirmation("work", "personal"):
            print("Switch cancelled. Maintaining current session.")
            return

        # First save current state as work
        save_current_state(WORK_STATE_FILE)
        print("Saved current state as work profile")
        ...
        # Load personal state
        if os.path.exists(PERSONAL_STATE_FILE):
            with open(PERSONAL_STATE_FILE, 'r') as f:
                state = json.load(f)
            ...
            print("Personal profile restored successfully!")
        else:
            print("No saved personal state found. Starting fresh personal session.")
    except Exception as e:
        print(f"Error restoring personal state: {e}")
